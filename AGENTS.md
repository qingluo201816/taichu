...
- **五层名称固定**：所有模型可见输入只能归入“稳定记忆、工作记忆、长期记忆、历史记忆、当前请求”五层之一；不得重新命名为“稳定背景、相关记忆、过程历史”等近义层，也不得在五层之外另造模型上下文分类。
- **稳定记忆**：包含系统身份、安全规则、固定权限规则、固定能力说明与输入输出 Schema、预算上限规则、输出 Schema 和返回格式等最不容易变化的应用规则。稳定记忆必须位于模型输入的稳定前缀，保持内容、顺序和序列化方式稳定，为后续 KV-cache 优化保留边界；本轮启用的工具集合、剩余预算、当前授权等运行实例值不属于稳定记忆。
...
- **长期记忆**：只表示跨任务沉淀并按当前请求召回的用户身份、表达习惯、写作偏好、协作偏好和效果反馈，用于优化 Agent 回答方式。长期记忆不是小说知识库、正文、外部资料或通用检索结果；单次临时要求不得自动提升为长期偏好。当前只保留分层边界，不新增治理字段，后续等 Agent Loop、反馈评测和自动沉淀机制稳定后再单独设计。
- **历史记忆**：历史事实源只永久保存已经发生的原始 `user.content` 和实际展示给用户的 `assistant.content`，包括 Human-in-the-loop 问答；历史模型投影由早期摘要、相关历史原文和近期原始消息组成，不默认重发完整对话。摘要只能用于延续语义，不能用于证明用户说过某句原话；摘要来源、覆盖范围和版本等治理字段暂不进入当前运行模型，待主链路稳定后再设计。历史记忆不得包含 `system`/`developer` 指令、工具定义、`assistant.tool_calls`、`tool` 结果、子 Agent 输入输出、DAG、节点状态、错误、重试、Token、预算、授权或输出 Schema。
...
- **API 角色不是记忆层**：太初统一模型契约使用 `system`、`developer`、`user`、`assistant`、`tool` 及原生 `tools` 参数表达来源和协议关系。`system/developer` 承载应用规则和组装上下文，`user` 只承载用户真实输入，`assistant` 保留历史模型正文或真实工具请求，`tool` 只回传与 `call_id` 配对的真实工具结果，固定工具定义通过 API 的 `tools` 参数传输。网关可以按厂商协议转换角色，但不得改变五层业务归属或把内部数据伪装成用户原话。
- **工具动态可用性**：固定工具说明、参数 Schema 和行为约束归稳定记忆并通过 `tools` 传输；本轮实际启用哪些工具、是否已授权、剩余调用次数和失败/重试状态归工作记忆。`assistant.tool_call(call_id)` 与对应 `tool(call_id)` 必须作为完整配对链保存在工作记忆或调用轨迹中，不得只保留工具结果；只有最终展示给用户的 `assistant.content` 才能在下一轮进入历史事实源。
