...
- 监控与评测入口的颜色必须表达稳定的业务归属，不得按“监控”“评测”等动作各自分配颜色。同一业务组在同页和跨页必须复用同一个低饱和语义图标色：知识沉淀监控与知识沉淀评测统一使用 `--tc-monitor-knowledge`，通用写作智能体监控与评测统一使用 `--tc-monitor-general-agent`，RAG 建模监控与统一召回评测统一使用 `--tc-monitor-rag`；不同业务组之间必须保持可辨认。精确色值只在主题 token 中维护。
