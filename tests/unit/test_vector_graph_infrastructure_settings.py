...
def test_vector_graph_defaults_use_milvus_and_local_embedding() -> None:
    ...
    assert settings.vector_graph_passage_top_k == 30
    assert settings.vector_graph_expansion_max_seed_entities == 5
    assert settings.vector_graph_expansion_max_seed_relations == 32
    assert settings.vector_graph_expansion_max_hop == 1
    assert settings.vector_graph_expansion_max_entities_per_hop == 20
    assert settings.vector_graph_expansion_relations_per_entity == 10
    assert settings.vector_graph_expansion_candidate_pool_multiplier == 4
    ...
    assert settings.vector_graph_expansion_beam_width == 24
    assert settings.vector_graph_expansion_max_total_relations == 56
    assert settings.vector_graph_expansion_max_graph_passages == 20
...
def test_vector_graph_service_is_registered_in_production_capabilities(
    ...
    assert (
        application.state.vector_graph_backend._milvus._llm._llm
        is application.state.chat_model
    )
    assert (
    ...
    assert expansion.max_seed_relations == 32
    assert expansion.max_hop == 1
    assert expansion.candidate_pool_multiplier == 4
    assert expansion.beam_width == 24
    assert expansion.max_total_relations == 56
