...
- **职责边界优先于短期便利**：不得以“当前实现简单”“暂时够用”或“以后再拆”为理由混合不同职责。已经明确属于不同层或不同变化原因的能力，必须从初始实现开始分离
- **插件发现与注册分离**：Agent 的扫描、动态导入等发现机制属于基础设施能力；协议校验、注册、查询属于应用层能力。二者必须通过明确接口协作，不得因扫描目标位于 `application/agents/` 而合并实现
- **领域层保持技术无关**：`domain` 可放技术无关的小说与产品域模型，但不得依赖具体 Agent 实现、LangGraph、LLM、MCP 或存储实现。
- **Agent 协议归属应用层**：Agent 协议定义放在应用层，不放入领域层。可扩展能力使用集合声明，例如 `required_capabilities = {"knowledge_search", "llm"}`；暴露渠道使用 `exposures = {"api", "ui", "mcp"}`，避免为每项能力持续增加布尔字段
- **跨层契约默认使用 Protocol**：存储、检索等行为契约优先使用 `typing.Protocol`，便于替换实现和测试注入；只有确实需要共享实现或固定生命周期模板时才使用 ABC
