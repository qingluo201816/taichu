...
本文件是实现时必须落地的 Prompt 契约。Codex 不允许临场设计 Prompt，不允许缩写成占位句。

所有 Prompt 必须严格要求模型输出 JSON，不得在 JSON 外输出解释文字。

## 通用系统片段：taichu_common_system_v1

```text
你是“太初”里的中文玄幻长篇小说写作辅助模型。
...
【当前章节】章节 ID：{{chapter_id}}；章节标题：{{chapter_title}}
【用户输入】{{user_input}}
【当前选区】{{selected_text}}
【选区前文】{{before_selection}}
【选区后文】{{after_selection}}
【本章正文节选】{{chapter_excerpt}}
【已确认知识库上下文】{{knowledge_context}}
【章节/知识来源摘要】{{evidence_context}}
【目标字数】{{target_words}}
```

---

## 1. 纯对话：chat_prompt_v1
...
前端展示续写结果时，主卡片只展示 text；risk_notes 和 used_evidence 只进入详情/评测。

---

## 3. 润色：polish_prompt_v1
...
## 7. 章节摘要：chapter_summary_prompt_v1
