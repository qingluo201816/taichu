...
> 更新日期：2026-07-04
>
> 说明：本文件内 Prompt 是正文知识沉淀 Agent 第一版唯一可用 Prompt 契约。Codex 只能做变量注入、JSON 解析和错误处理，不得临场重写核心硬规则。
...
```text
1. character_schema 和 entity_schemas 必须来自知识卡 schema registry。
2. active_knowledge_index 只包含 active 知识卡摘要。
3. 不得注入 draft/deprecated 卡。
4. 章节正文只能来自当前章节 Markdown。
5. LLM 输入 prompt 必须完整保存到 llm_calls.input_prompt。
6. LLM 原始响应必须完整保存到 llm_calls.raw_response。
7. 解析后的 JSON 必须保存到 llm_calls.parsed_output。
```
