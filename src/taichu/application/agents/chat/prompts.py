...
def build_chat_prompt(
    *,
    ...
    source_rule = (
        ...
        "内容必须写明“以下为推测”。不得把待确认设定、灵感、智能助手卡片或"
        "章节摘要当作正式事实。"
    )
    ...
    return "\n\n".join(
        [
            SYSTEM_PROMPT,
            source_rule,
            _numbered_block("可用来源", source_lines),
            f"作者问题：{message}",
        ]
    )


def _numbered_block(label: str, lines: list[str]) -> str:
    if not lines:
        return f"{label}：无"
    numbered = [f"[S{index + 1}] {line}" for index, line in enumerate(lines)]
    return f"{label}：\n" + "\n".join(numbered)
