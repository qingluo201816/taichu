...
监控入口中的“执行轨迹”（`/task-monitor/general-agent/trajectory`）采用全宽连续账本：顶部三条微型时间轨可按时长、轮次或调用顺序查看，下方紧凑展示助手消息、结构化输出与工具输入→输出。支持对话选择、单轮或全部轮次、摘要搜索、类型筛选、消息回放/调用追踪视角、点击定位、按需证据抽屉与运行中刷新；长列表通过窗口化渲染连续滚动，不按页切断。页面只读取现有运行记录、调用追踪、上下文快照和脱敏模型回放；原生工具结果按同轮调用编号严格配对，结构化输出不当作外部工具，冲突或缺失反馈明确标注。不同编号的追踪与回放不猜测关联，缺失时间不估算，界面推断的中断状态明确标注。首字延迟与解码耗时尚未采集，不提供分叉或重执行操作。
