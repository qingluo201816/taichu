...
|---|---|---|---|---|
| `stable_background`（稳定背景） | 告诉模型它是谁、系统边界是什么 | Agent 角色、事实源规则、工具与子 Agent 使用原则 | 内容变化慢 | 最晚裁 |
| `working_memory`（工作记忆） | 告诉模型当前任务已经做到哪 | 用户约束、任务摘要、资源摘要、节点摘要、未决问题、当前计划摘要 | 按会话自动维护 | 较晚裁 |
| `related_memories`（相关记忆） | 为当前请求补回少量最相关旧信息 | 词法相关的运行记忆 | 每次按需选择 | 最先裁 |
| `process_history`（过程历史） | 提供前面交流和执行过程的必要证据 | 最近消息、必要的历史摘要 | 按当前预算取用 | 较早裁 |
...
2. 明确的作者硬约束自动写入运行记忆；
3. 固定的系统边界进入 `stable_background`（稳定背景）；
4. 规划前生成第一份 `context_snapshot_id`（上下文快照标识）；
5. 节点成功、校验完成或出现未决问题后，工作记忆继续被结构化更新。
...
1. 先删除低优先级 `related_memories`（相关记忆）；
2. 再把 `process_history`（过程历史）压成 `ContextDigest`（上下文摘要），之后才从最旧消息继续裁；
3. 再按顺序收缩工作笔记、资源摘要、任务摘要、未决问题、用户指令；
4. 然后收缩节点摘要和计划摘要；
5. 最后才收缩 `stable_background`（稳定背景），并至少保留一项核心边界；
6. `current_request`（当前请求）不截断。如果其他层全部裁完仍容不下当前请求，明确失败。
...
当前规划器采用两阶段披露，所有已注册工具和子 Agent 都能先进入轻量目录，不存在“候选能力最多只加载 16 个”的当前限制。

第一阶段：为全部能力提供轻量摘要。

- 工具：名称、类型、中文职责、是否有副作用、是否外部调用、授权方式；
- 子 Agent：名称、类型、中文名称、职责、明确不负责什么、允许调用哪些工具、接受和产出哪些任务产物类型。

第二阶段：规划器选出本次最小充分能力集合后，只为已选能力加载完整 `JSON Schema`（JSON 参数结构约束），再填写精确参数和节点输入绑定。
...
因此当前设计是：**能力目录全量可见，完整契约按选择渐进加载。** 这不是少给能力，而是把“发现能力”和“调用能力”分成两步。

### 11.3 两阶段都有什么硬保护

- 轻量目录和已选契约分别有约 80,000 字符预算；
- 超预算时明确报错，不允许无声丢能力；
- 规划骨架确定后，第二阶段只能补充 `input_data`（节点静态输入）与 `input_bindings`（上游输出绑定），不能偷偷换能力或改依赖；
...
规划器先生成不含精确参数的能力选择骨架，再根据已选能力的完整契约生成可执行计划。一个节点至少包括：
...
上游子 Agent 产出的 `artifact_refs`（任务产物引用）会被执行器注入下游子 Agent 的 `source_request.upstream_artifact_refs`（来源请求中的上游任务产物引用）。下游读取的是已持久化、带哈希的产物，而不是依赖模型在过程历史里“记得上一步说了什么”。
...
所以它证明“检查点恢复骨架和确定性矩阵可用”，不证明“所有真实长任务已经稳定”。阶段结果见任务包中的阶段 06 文档与 [`7-19通用写作助手运行时记忆与上下文压缩报告.md`（运行记忆与上下文历史报告）](../历史/7-19通用写作助手运行时记忆与上下文压缩报告.md)，但恢复实现应以当前源码为准。
...
| 相关记忆就是知识向量召回 | 相关运行记忆目前用独立词法索引；小说知识召回是另一条服务 |
