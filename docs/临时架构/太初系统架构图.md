...
| 事实查询 | 正文 Markdown + MongoDB confirmed 直接查询 | 如未来引入向量、Elasticsearch 或图能力，只能作为可重建派生层 | 绿色事实链与灰色预留节点 |
...
5. SQLite/FTS 已废弃，不参与后续决策；未来若引入向量、Elasticsearch、图能力或缓存，也只能作为可重建派生层。
